...
def main():
    ...
    if not cif_files:
        # AF3 creates timestamped sibling dirs when the base dir already exists
        # from a prior failed run: {af3_output_dir}/{design}_{timestamp}/ instead of
        # {af3_output_dir}/{design}/. Search those siblings too.
        for sibling in sorted(glob(os.path.join(args.af3_output_dir, f"{args.design}_*"))):
            if os.path.isdir(sibling):
                cif_files.extend(sorted(glob(
                    os.path.join(sibling, "**", "*_model.cif"), recursive=True)))
        cif_files.sort()
