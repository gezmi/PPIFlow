...
def extract_chain_sequence(full_sequence: str, pose, chain_id: str) -> str:
    ...
    # If FASTA contains only the designed chain (not full multi-chain concat)
    if len(full_sequence) == length:
        return full_sequence, chain_start, chain_end
...
def find_backbone_for_fasta(fasta_name: str, backbone_dir: str) -> str:
    ...
    for suffix in ["_0001", "_redesigned", "_hydro"]:
...
def main():
    ...
    parser.add_argument("--batch_idx", type=int, default=None,
                        help="Process only fasta_files[batch_idx::num_batches]")
    parser.add_argument("--num_batches", type=int, default=None,
                        help="Total number of batches (used with --batch_idx)")
    ...
    # Batch slicing: process only a subset of FASTA files
    if args.batch_idx is not None and args.num_batches is not None:
        fasta_files = fasta_files[args.batch_idx::args.num_batches]
        print(f"Batch {args.batch_idx}/{args.num_batches}: "
              f"processing {len(fasta_files)} FASTA files")
